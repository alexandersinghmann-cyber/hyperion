...
(function(){
  ...
  // GOOD → session starts synchronously, status stamped (Gym day, index 0 — v13 Monday)
  _backStartIdx=0;setBackStatus('good');
  ...
  const _progLen=S.program.days[0].exercises.length;
  _backStartIdx=0;setBackStatus('tight');
  ...
  assert(S.program.days[0].exercises.length===_progLen, 'K6: the add-on is session-only — program untouched');
  cancelSession();
  // FLARE → recovery day minted + started; original day untouched and pending
  const _daysN=S.program.days.length;
  _backStartIdx=0;setBackStatus('flare');
  ...
  assert(isDayDone(0)===false&&!S.program.days[0].scheduledDate, 'K6: the planned day stays pending and reschedulable (no failure state)');
...
const tEst = estimateSessionTime(dayExercises(pa_d1,'A').map(e=>({name:e.name,sets:e.sets,reps:e.reps,rest:e.rest,entryType:e.entryType,minutes:e.minutes})));
// Sanity bounds are about catching pathological estimates, not enforcing budget.
assert(tEst >= 60 && tEst <= 120, 'Phase B/R2: Gym (Week A) estimate in sane range. Got: ' + tEst);
...
const freshD1 = JSON.parse(JSON.stringify(DEF_PROGRAM)).days.find(d=>d.id===1); // v13: Monday gym (shared alternating definition)
...
assert(variantKey(DEF_PROGRAM.days.find(d=>d.id===1).exercises.find(e=>e.name==='Cable Low Row').variant)===variantKey({pulley:'single'}), 'Variant: R2 Cable Low Row prescribed SINGLE-pulley (no grip authored this block)');
...
S.program=JSON.parse(JSON.stringify(DEF_PROGRAM));migrateV3();
// v13 week is warn-clean: the Sun→Mon gym adjacency is accepted-as-info via
// the authored 'adjacency-accepted' tags (asserted in Phase B/R2 above).
const vwDeload=validateWeek(weekDatesFor('2026-07-13'),'2026-07-13');
assert(vwDeload.reduce((a,r)=>a+r.warnings.filter(w=>w.level==='warn').length,0)===0, 'D5: v13 week validates warn-clean. Got: '+JSON.stringify(vwDeload.flatMap(r=>r.warnings.filter(w=>w.level==='warn').map(w=>w.msg))));
...
S.program=JSON.parse(JSON.stringify(DEF_PROGRAM));migrateV3();S.sessions=[];
global.startTimer=()=>{};global.showSessionHero=global.showSessionHero||(()=>{});
startDay(0); // R2 Gym Monday (Back Squat present in BOTH parities)
...
(()=>{
  ...
  startDay(0); // v13 Gym Monday (index 0)
  ...
  const dlDay=S.program.days[0].exercises.find(e=>e.name==='Trap Bar Deadlift');
  delete dlDay.warmup;
  startDay(0);
...
(function(){
  ...
  S.activeSession={dayIndex:0,dayId:1,date:todayStr(),dayLabel:'Gym',sessionType:'lifting',startTime:1,exercises:[
  ...
  assert(S.leftovers[0].fromDayId===1&&S.leftovers[0].prescribed.loadKg===12.5, 'L2: carries origin + prescription snapshot');
  // toggle OFF → no entry
  ...
  S.sessions=[{id:'s_orig',date:todayStr(),dayLabel:'Gym',dayId:1,blockName:S.program.name,sessionType:'lifting',duration:60,rpe:7,status:'complete',exercises:[],painEvents:[]}];
  S.activeSession=null;
...
(function(){
  ...
  S.program=JSON.parse(JSON.stringify(DEF_PROGRAM));
  S.program.days[0].exercises.find(e=>e.name==='Back Extension').cue='Neutral spine only \u2014 no hyperextension';
  S.program.days[1].note='Freeform KB \u2014 user-programmed. Log duration, bells, and what you did.';
  S.settings._cuePressL3=false;
  migrateV3();
  assert(/Pad at hip crease/.test(S.program.days[0].exercises.find(e=>e.name==='Back Extension').cue), 'L3: one-shot installs the setup cue on the live copy (no version bump)');
  ...
  assert(!/Press re-entry/.test(S.program.days[1].note), 'L3/M1: KB note no longer gains the re-entry line');
...
(function(){
  ...
  migrateV3();
  const gd=S.program.days[0],exN=gd.exercises.map(e=>e.name);
  assert(!exN.includes('KB Press'), 'M1: KB Press removed from the gym day (classes are the overhead dose)');
  assert(!/Press re-entry/.test(S.program.days[1].note)&&/Log duration, bells/.test(S.program.days[1].note), 'M1: KB note stripped of the re-entry line, rest kept');
...
assert(DEF_PROGRAM.days[0].exercises.some(e=>e.name==='Iso-Lateral Bench Press')&&DEF_PROGRAM.days[0].exercises.some(e=>e.name==='Pallof Press'), 'M1: DEF gym day carries Iso-Lateral Bench + Pallof');
assert(DEF_PROGRAM.days.every(d=>!/Press re-entry/.test(d.note||'')), 'M1: no DEF day note carries the re-entry line');
...
assert(/Pad at hip crease .* rise to a straight line ONLY/.test(DEF_PROGRAM.days[0].exercises.find(e=>e.name==='Back Extension').cue)&&DEF_PROGRAM.days[0].exercises.find(e=>e.name==='Back Extension').frozen===true, 'L3: DEF cue updated, FROZEN kept');
